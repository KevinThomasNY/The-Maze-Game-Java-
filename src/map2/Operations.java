        ...
        if (!changeable_key) return;
        	   ...
        	   if (m_currex + 1 == m - 1 && m_currey == n - 1) {
                  ...
                  map.timeExit=true;
                   ...
                   map2.map.gtThread.exit=true;
            		map3.map.cover = new JPanel();
            	    new map3.map(22,22,true,1);
                   changeable_key = true;    
                   setEditable(false);      
                   }
               }
    ...
    public static void left() throws Exception {
        ...
        if (Operations.m_startx3_1 == Operations.m_currex && Operations.m_starty3_1+1 == Operations.m_currey) {
          ...
          if(num==1) {
        	  map.timeThread.suspend();
        	    map.gtThread.suspend();
        	   map.timeThread.suspend();String[] options= {"Ok"};  JOptionPane.showOptionDialog(null, "You found the key, now you need to find the exit!", null,JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
        	    map.timeThread.resume();
