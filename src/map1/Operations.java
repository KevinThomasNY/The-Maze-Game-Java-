...
public class Operations {
	static boolean monsterIsStop =false;
    static int m, n;   
    static team_about t;
    ...
    public  static void math(int a_x, int a_y, int b_x, int b_y) {
    	...
    	if( a_x==b_x&&a_y==b_y){
    		int a_x1 =(int) (Math.random() * m / 2);
    		int a_y1 =(int) (Math.random() * m / 2);
    		 math(a_x1,a_y1,b_x,b_y);
    	}
        ...
        if (!changeable_key) return;
        	   ...
        	   if (m_currex + 1 == m - 1 && m_currey == n - 1) {
                   ...
                   if(num==1){
                	   ...
                	   map.timeThread.suspend();
                	    map.gtThread.suspend();
                   String[] options= {"Ok"};  JOptionPane.showOptionDialog(null, "Next Level", null,JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
               ...
               if (Operations.m_startx3_1-1 == Operations.m_currex && Operations.m_starty3_1 == Operations.m_currey) {
            	   ...
            	   map.tp[m_startx3_1-1][m_starty3_1].change(1);
            	   	...
            	   	restart = true;
            	    if(num==1) {map.timeThread.suspend();
            	    map.gtThread.suspend();
    ...
    public static void left() throws Exception {
        ...
        if (m_currey - 1 >= 0 && !map.tp[m_currex][m_currey - 1].isWall()) {
            ...
            if (map.m_currentx2 == Operations.m_currex && Operations.m_starty2 == Operations.m_currey) {
            	map.tp[m_currex][m_starty2].change(4);
            	eat();
            }
...
//    	  JOptionPane.showMessageDialog(null, "OK", "Game Rules", JOptionPane.INFORMATION_MESSAGE);
         t =new team_about();
        t.setVisible(true);
//    	int m = JOptionPane.showOptionDialog(null, "Watch Over", "Ok?",JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
